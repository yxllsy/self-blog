...
<template>
  <div class="container">
    ...
    <div class="container_content" :style="{ display: isMobileOrPc() ? 'block' : 'flex', height: isMobileOrPc() ? '86vh' : '100vh'}">
      <template v-if="isMobileOrPc()">
        <userLayout v-if="route.name !== 'Article'" class="user_layout" :isMobile="true" style="width: calc(100vw - 32px); margin-right: 16px; max-height: 27vh;" />
        <div class="layout">
          <router-view />
        </div>
      </template>
      <template v-else>
        ...
        <userLayout  class="user_layout" :style="{maxHeight: isMobileOrPc() ? '35vh' : '41vh'}" />
      </template>
    </div>
    <BgLayer />
  </div>
